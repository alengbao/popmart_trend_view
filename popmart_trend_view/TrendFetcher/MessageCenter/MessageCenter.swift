...
class MessageManager: ObservableObject {
    ...
    func sendMessage(title: String, body: String) {
        addInAppMessage(body)
        sendPushNotification(title, body: body)
    }
    
    // 添加站内信
    func addInAppMessage(_ content: String) {
        let messageType = determineMessageType(from: content)
        let message = InAppMessage(content: content, type: messageType)
        self.inAppMessages.insert(message, at: 0)
        // 限制站内信数量，最多保留20条
        if self.inAppMessages.count > maxMessageCount {
            self.inAppMessages = Array(self.inAppMessages.prefix(maxMessageCount))
        }
    }
    
    // 添加带类型的站内信
    func addInAppMessage(_ content: String, type: MessageType) {
        let message = InAppMessage(content: content, type: type)
        self.inAppMessages.insert(message, at: 0)
        // 限制站内信数量，最多保留20条
        if self.inAppMessages.count > maxMessageCount {
            self.inAppMessages = Array(self.inAppMessages.prefix(maxMessageCount))
    ...
    // 根据内容自动判断消息类型
    private func determineMessageType(from content: String) -> MessageType {
        let lowercasedContent = content.lowercased()
        
        if lowercasedContent.contains("强烈卖出") || lowercasedContent.contains("强烈看跌") {
            return .strongSell
        } else if lowercasedContent.contains("卖出") || lowercasedContent.contains("看跌") {
            return .normalSell
        } else if lowercasedContent.contains("强烈买入") || lowercasedContent.contains("强烈看涨") {
            return .strongBuy
        } else if lowercasedContent.contains("买入") || lowercasedContent.contains("看涨") {
            return .normalBuy
        } else {
            return .neutral
        }
